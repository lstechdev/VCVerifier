...
func VerifierAPIAuthenticationResponse(c *gin.Context) {
	...
	if err != nil {
		logging.Log().Infof("Was not able to decode the form string %s. Err: %v", base64Token, err)
	...
	if err != nil {
		logging.Log().Infof("Was not able to decode the credentials from the token %s. Err: %v", base64Token, err)
		c.AbortWithStatusJSON(400, ErrorMessageUnableToDecodeCredential)
		return
	}
	err = json.Unmarshal(rawHolder, &holder)
	if err != nil || holder.Id == "" {
		logging.Log().Infof("Was not able to decode the holder from the token %s. Err: %v", base64Token, err)
...
func VerifierAPIJWKS(c *gin.Context) {
	c.JSON(http.StatusOK, apiVerifier.GetJWKS())
}
