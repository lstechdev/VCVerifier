...
func TestVerifyVC_Participant(t *testing.T) {
	...
	tests := []test{
		...
		{testName: "If no registry is configured, the credential should be accepted.", credentialToVerifiy: VerifiableCredential{raw: map[string]interface{}{"issuer": "did:web:trusted-issuer.org"}}, verificationContext: TrustRegistriesVerificationContext{trustedParticipantsRegistries: map[string][]string{}}, tirResponse: true, expectedResult: true},
		{testName: "If no registry is configured, the credential should be accepted.", credentialToVerifiy: VerifiableCredential{raw: map[string]interface{}{"issuer": "did:web:trusted-issuer.org"}}, verificationContext: TrustRegistriesVerificationContext{trustedParticipantsRegistries: map[string][]string{}}, tirResponse: false, expectedResult: true},
